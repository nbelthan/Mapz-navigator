...
# Read the California GeoJSON file
california_geojson = gpd.read_file('SFBA_Shape.geojson')

# Read the shapefile for the zip code boundaries
zip_code_boundaries = gpd.read_file('zip_codes.geojson')

# Define the zip codes of interest
...
# Create a plot for each zip code
for i, zip_code in enumerate(zip_codes, start=1):
    # Filter the zip code boundaries for the current zip code
    zip_boundary = zip_code_boundaries[zip_code_boundaries['zcta'] == zip_code]

    # Create a plot of the map
    fig, ax = plt.subplots(figsize=(20, 20))

    # Plot the topographical map of California using contextily
    california_geojson.to_crs(epsg=3857).plot(ax=ax, color='none', alpha=0.05)
    ctx.add_basemap(ax, source=ctx.providers.Esri.WorldStreetMap, alpha=0.5)

    # Plot the zip code boundary
    zip_boundary.to_crs(epsg=3857).boundary.plot(ax=ax, facecolor='lightpink', edgecolor='red', linewidth=1)
    ...
    # Save the figure as a PNG file in memory buffer
    buffer = io.BytesIO()
    ...
    # Convert to JPEG with compression quality (80) and save in memory buffer
    buffer = io.BytesIO()
    image.save(buffer, format='jpeg', quality=80)
    buffer.seek(0)

    # Save the image buffer to a file
    file_name = f'BAZip/{zip_code}.jpg'
    with open(file_name, 'wb') as f:
        f.write(buffer.read())
        print(f'Saved image for zip code {zip_code} as {file_name}')
        plt.close(fig)
